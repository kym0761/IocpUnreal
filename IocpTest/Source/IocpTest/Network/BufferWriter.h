#pragma once
class FIocpBufferWriter
{
public:
	FIocpBufferWriter();
	FIocpBufferWriter(BYTE* buffer, uint32 size, uint32 pos = 0);
	~FIocpBufferWriter();
	...
	template<typename T>
	FIocpBufferWriter& operator<<(T&& src);
...
template<typename T>
T* FIocpBufferWriter::Reserve(uint16 count)
{
...
//write operator
template<typename T>
FIocpBufferWriter& FIocpBufferWriter::operator<<(T&& src) //보편 참조
{
