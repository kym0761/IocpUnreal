...
namespace SP
{
    ...
    class InsertGold : public FDBBind<3,0>
    {
    public:
    	InsertGold(FDBConnection& conn) : FDBBind(conn, L"{CALL dbo.spInsertGold(?,?,?)}") { }
    ...
    class GetGold : public FDBBind<1,4>
    {
    public:
    	GetGold(FDBConnection& conn) : FDBBind(conn, L"{CALL dbo.spGetGold(?)}") { }
    	void In_Gold(int32& v) { BindParam(0, v); };
    	void In_Gold(int32&& v) { _gold = std::move(v); BindParam(0, _gold); };
